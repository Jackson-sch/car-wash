...
const CustomTooltip = ({ active, payload, label, mode }: { active?: boolean; payload?: { value: number | string }[]; label?: string; mode?: string }) => {
    ...
    return (
      <div className="bg-popover text-popover-foreground border border-border p-3 rounded-lg shadow-xl text-xs">
        ...
        <p className="font-extrabold text-secondary text-sm">
          {mode === "revenue" ? formatCurrency(Number(value)) : `${value} servicios`}
        </p>
      </div>
    );
  }
...
export function ProductivityChartInner({ data, rol: _rol }: ProductivityChartProps) {
    ...
    <ChartSkeleton>
      <div className="flex justify-between">
        <div className="space-y-2">
          <div className="h-3 w-52 bg-muted/40 rounded animate-pulse" />
          <div className="h-2 w-40 bg-muted/30 rounded animate-pulse" />
        </div>
        <div className="flex gap-2">
          <div className="h-7 w-20 bg-muted/30 rounded-md animate-pulse" />
          <div className="h-7 w-20 bg-muted/30 rounded-md animate-pulse" />
        </div>
      </div>
    </ChartSkeleton>
