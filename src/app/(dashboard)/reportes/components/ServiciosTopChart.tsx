      ...
      <div className="bg-popover text-popover-foreground border border-border p-3 rounded-lg shadow-xl text-xs">
        ...
        <div className="space-y-0.5">
          <p className="text-muted-foreground font-bold text-[10px]">
            Servicios: <span className="text-foreground font-extrabold">{payload[0].value}</span>
          </p>
          <p className="text-secondary font-bold text-[10px]">
            Recaudación: <span className="font-extrabold">S/ {parseFloat(payload[0].payload.total || "0").toFixed(2)}</span>
          </p>
        </div>
      </div>
    );
...
export function ServiciosTopChart({ serviciosTop }: ServiciosTopChartProps) {
              ...
              <Bar dataKey="cantidad" radius={[6, 6, 0, 0]} maxBarSize={45}>
                {serviciosTop.map((entry, index) => (
                  <Cell
                    ...
                    onMouseEnter={(e: any) => onMouseEnter(e, index)}
