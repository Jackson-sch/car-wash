...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: { name?: string; value?: number | string }[] }) => {
    ...
    return (
      <div className="bg-popover text-popover-foreground border border-border p-3 rounded-lg shadow-xl text-xs">
        ...
        <p className="font-black text-secondary">
          S/ {parseFloat(String(payload[0].value ?? 0)).toFixed(2)}
        </p>
      </div>
    );
  }
...
export function MetodosPagoChartInner({ pagosMetodo }: MetodosPagoChartProps) {
  ...
  if (!Recharts) return (
    <ChartSkeleton>
      ...
      <div className="h-80 w-full flex items-center justify-center">
        <div className="w-40 h-40 rounded-full bg-muted/20 animate-pulse" />
      </div>
    </ChartSkeleton>
  );

  const { ResponsiveContainer, PieChart, Pie, Cell, Sector: RechartsSector, Tooltip, Legend } = Recharts;

  const renderShape = (props: { cx?: number; cy?: number; innerRadius?: number; outerRadius: number; startAngle?: number; endAngle?: number; fill?: string; isActive?: boolean }) => {
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Legend
                  ...
                  iconSize={8}
                  formatter={(val: string) => <span className="text-[10px] text-muted-foreground font-bold">{val}</span>}
