...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    const actual = payload.find((p: any) => p.dataKey === "cantidad")?.value ?? 0;
    const projected = payload.find((p: any) => p.dataKey === "prediccion")?.value ?? 0;
...
export function HorasPicoChart({ horasPico }: HorasPicoChartProps) {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);

  // Determinar la hora pico real basada en datos reales o proyectados
